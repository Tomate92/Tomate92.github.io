...
function ajouterUtilisateur(username,country,email, password) {
    let transaction = db.transaction(["users"], "readwrite");
    let store = transaction.objectStore("users");
    let request = store.add({ username: username , email: email, password: password ,  country: country});
...
document.getElementById("login-form").onsubmit = function(event) {
    ...
    // let firstname = document.getElementById("Pseudo").value;
    ...
    if(country === "" || username === "" || email === "" || password === ""){
        ...
        if (!email.includes('@')){
            ...
            if (email.includes('.net') || email.includes('.com') || email.includes('.fr') || email.includes('.io')) {
                ajouterUtilisateur(username, country, email, password);
